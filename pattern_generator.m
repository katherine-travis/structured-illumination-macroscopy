...
%% Import an image 
img = rgb2gray(imread('/Users/katherinetravis/Documents/448_data/USAF_Resolution.jpeg'));
p=0.01;  % p% of the top dominant frequencies will be found
...
% Identify dominant frequencies (top x% by magnitude)
...
for k = 1:length(dominantIdx)
    ...
    pattern = cos(2 * pi * (u * X / size(img, 2) + v * Y / size(img, 1)) + phase_shift);
    ...
    filename = sprintf('pattern_freq_%.2f_phase_%.2f.png', freq, phase_shift);
